...
public record EntityDimensions(float width, float height, float eyeHeight, EntityAttachments attachments, boolean fixed) {
    ...
    public EntityDimensions withEyeHeight(float p_316487_) {
        return new EntityDimensions(this.width, this.height, p_316487_, this.attachments, this.fixed);
    }

    public EntityDimensions withAttachments(EntityAttachments.Builder p_316576_) {
        return new EntityDimensions(this.width, this.height, this.eyeHeight, p_316576_.build(this.width, this.height), this.fixed);
    }
}
