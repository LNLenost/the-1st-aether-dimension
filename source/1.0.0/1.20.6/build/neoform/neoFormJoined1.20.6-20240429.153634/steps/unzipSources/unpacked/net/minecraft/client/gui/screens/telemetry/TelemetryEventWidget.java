...
public class TelemetryEventWidget extends AbstractScrollWidget {
    ...
    private Component grayOutIfDisabled(Component p_308894_, boolean p_308910_) {
        return (Component)(p_308910_ ? p_308894_.copy().withStyle(ChatFormatting.GRAY) : p_308894_);
    }

    private void addEventType(TelemetryEventWidget.ContentBuilder p_261823_, TelemetryEventType p_262127_, boolean p_308993_) {
        String s = p_262127_.isOptIn() ? (p_308993_ ? "telemetry.event.optional.disabled" : "telemetry.event.optional") : "telemetry.event.required";
        p_261823_.addHeader(this.font, this.grayOutIfDisabled(Component.translatable(s, p_262127_.title()), p_308993_));
        p_261823_.addHeader(this.font, p_262127_.description().withStyle(ChatFormatting.GRAY));
        p_261823_.addSpacer(9 / 2);
        p_261823_.addLine(this.font, this.grayOutIfDisabled(PROPERTY_TITLE, p_308993_), 2);
        this.addEventTypeProperties(p_262127_, p_261823_, p_308993_);
    }
    ...
    private void addEventTypeProperties(TelemetryEventType p_262105_, TelemetryEventWidget.ContentBuilder p_261932_, boolean p_308932_) {
        for (TelemetryProperty<?> telemetryproperty : p_262105_.properties()) {
            p_261932_.addLine(this.font, this.grayOutIfDisabled(telemetryproperty.title(), p_308932_));
        }
    }
    ...
    static class ContentBuilder {
        ...
        public void addLine(Font p_261894_, Component p_261816_, int p_261721_) {
            this.layout.addChild(new MultiLineTextWidget(p_261816_, p_261894_).setMaxWidth(this.width), p_295098_ -> p_295098_.paddingBottom(p_261721_));
