...
public class VertexMultiConsumer {
    ...
    static class Double implements VertexConsumer {
        ...
        public void vertex(
            ...
            this.first
                .vertex(
                    p_86184_, p_86185_, p_86186_, p_86187_, p_86188_, p_86189_, p_86190_, p_86191_, p_86192_, p_86193_, p_86194_, p_86195_, p_86196_, p_86197_
                );
            this.second
                .vertex(
                    p_86184_, p_86185_, p_86186_, p_86187_, p_86188_, p_86189_, p_86190_, p_86191_, p_86192_, p_86193_, p_86194_, p_86195_, p_86196_, p_86197_
    ...
    static class Multiple implements VertexConsumer {
        ...
        public VertexConsumer vertex(double p_167075_, double p_167076_, double p_167077_) {
            this.forEach(p_167082_ -> p_167082_.vertex(p_167075_, p_167076_, p_167077_));
            return this;
        }

        @Override
        public VertexConsumer color(int p_167130_, int p_167131_, int p_167132_, int p_167133_) {
            this.forEach(p_167163_ -> p_167163_.color(p_167130_, p_167131_, p_167132_, p_167133_));
            return this;
        }

        @Override
        public VertexConsumer uv(float p_167084_, float p_167085_) {
            this.forEach(p_167125_ -> p_167125_.uv(p_167084_, p_167085_));
            return this;
        }

        @Override
        public VertexConsumer overlayCoords(int p_167127_, int p_167128_) {
            this.forEach(p_167167_ -> p_167167_.overlayCoords(p_167127_, p_167128_));
            return this;
        }

        @Override
        public VertexConsumer uv2(int p_167151_, int p_167152_) {
            this.forEach(p_167143_ -> p_167143_.uv2(p_167151_, p_167152_));
            return this;
        }

        @Override
        public VertexConsumer normal(float p_167147_, float p_167148_, float p_167149_) {
            this.forEach(p_167121_ -> p_167121_.normal(p_167147_, p_167148_, p_167149_));
            return this;
        }
        ...
        public void defaultColor(int p_167154_, int p_167155_, int p_167156_, int p_167157_) {
            this.forEach(p_167139_ -> p_167139_.defaultColor(p_167154_, p_167155_, p_167156_, p_167157_));
        }
