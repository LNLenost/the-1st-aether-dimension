...
public record ClientboundCommandSuggestionsPacket(int id, int start, int length, List<ClientboundCommandSuggestionsPacket.Entry> suggestions)
    ...
    public ClientboundCommandSuggestionsPacket(int p_131846_, Suggestions p_131847_) {
        this(
            ...
            p_131847_.getList()
                .stream()
                .map(
                    p_323141_ -> new ClientboundCommandSuggestionsPacket.Entry(
                            p_323141_.getText(), Optional.ofNullable(p_323141_.getTooltip()).map(ComponentUtils::fromMessage)
                        )
                )
                .toList()
        );
    ...
    public Suggestions toSuggestions() {
        ...
        return new Suggestions(
            stringrange, this.suggestions.stream().map(p_323143_ -> new Suggestion(stringrange, p_323143_.text(), p_323143_.tooltip().orElse(null))).toList()
        );
    }
