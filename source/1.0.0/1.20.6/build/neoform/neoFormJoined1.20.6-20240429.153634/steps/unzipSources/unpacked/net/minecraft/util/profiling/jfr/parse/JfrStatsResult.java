...
public record JfrStatsResult(
    ...
    public List<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>> chunkGenSummary() {
        ...
        return map.entrySet()
            .stream()
            .map(p_329989_ -> Pair.of(p_329989_.getKey(), TimedStatSummary.summary(p_329989_.getValue())))
            .sorted(
                Comparator.<Pair<ChunkStatus, TimedStatSummary<ChunkGenStat>>, Duration>comparing(p_185507_ -> p_185507_.getSecond().totalDuration())
                    .reversed()
            )
            .toList();
    }
